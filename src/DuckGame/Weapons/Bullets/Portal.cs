  ...
  public class Portal : Thing
  {
      ...
      Vec2 BottomRight)
    {
      ...
      foreach (PortalDoor door in this._doors)
      {
        ...
        {
          foreach (Block block in door.collision)
          {
            if (Collision.Rect(TopLeft, BottomRight, (Thing) block))
              source.Add((MaterialThing) block);
          }
        }
      }
    ...
    public override void Update()
    {
      ...
      using (IEnumerator<ITeleport> enumerator = teleports.GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
          ITeleport t = enumerator.Current;
