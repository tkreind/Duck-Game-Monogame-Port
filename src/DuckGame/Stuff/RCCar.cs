...
namespace DuckGame
{
  ...
  public class RCCar : Holdable, IPlatform
  {
    ...
    private SinWave _wave = new SinWave(0.1f);
    ...
    protected override bool OnDestroy(DestroyType type = null)
    {
      ...
      for (int index = 0; index < 20; ++index)
      {
        ...
        Bullet bullet = new Bullet(this.x + (float) (Math.Cos((double) Maths.DegToRad(num)) * 6.0), this.y - (float) (Math.Sin((double) Maths.DegToRad(num)) * 6.0), (AmmoType) atrcShrapnel, num);
    ...
    public override void Update()
    {
      ...
      if (!this.isServerForObject || (double) this.y <= (double) Level.current.lowestPoint + 100.0 || this.destroyed)
        return;
