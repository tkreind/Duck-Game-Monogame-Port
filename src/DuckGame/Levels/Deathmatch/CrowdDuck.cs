...
namespace DuckGame
{
  public class CrowdDuck : Thing
  {
    ...
    public override void Draw()
    {
      ...
      {
        SpriteMap spriteMap = this._lastLoyalty.persona.defaultHead;
        Vec2 vec2 = Vec2.Zero;
        if (this._lastLoyalty.team.hasHat)
        {
          vec2 = this._lastLoyalty.team.hatOffset;
          spriteMap = this._lastLoyalty.team.hat;
        }
        if (spriteMap == null)
          return;
        spriteMap.depth = this.depth + 2;
        spriteMap.angle = 0.0f;
        spriteMap.alpha = 1f;
        ...
        if (spriteMap.flipH)
          vec2.x = -vec2.x;
        if (this.loyal)
          spriteMap.frame = flag ? 1 : 0;
        spriteMap.CenterOrigin();
        Graphics.Draw((Sprite) spriteMap, (float) ((double) this.x - (double) vec2.x + 8.0) + num, (float) ((double) this.y - (double) vec2.y - 22.0 - (flag ? 1.0 : 0.0)));
        spriteMap.frame = 0;
        spriteMap.flipH = false;
      }
      base.Draw();
