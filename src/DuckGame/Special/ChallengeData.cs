...
namespace DuckGame
{
  public class ChallengeData : Serializable
  {
    ...
    {
      XElement element = new XElement((XName) "challengeData");
      ...
      foreach (ChallengeTrophy trophy in this._trophies)
        element.Add((object) trophy.LegacySerialize());
      return element;
    }
    ...
    public bool LegacyDeserialize(XElement node)
    {
      ...
      int index = 0;
      foreach (XElement element in node.Elements((XName) "challengeTrophy"))
      {
        ChallengeTrophy challengeTrophy = new ChallengeTrophy(this);
