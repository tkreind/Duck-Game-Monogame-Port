...
namespace DuckGame
{
  public class MessageDialogue : ContextMenu
  {
    ...
    public override void Draw()
    {
      ...
      this._font.Draw("OK", (float) ((double) p1_2.x + (double) vec2_1.x / 2.0 - (double) this._font.GetWidth("OK") / 2.0), p1_2.y + 12f, Color.White, this.depth + 3);
      Vec2 p1_3 = p1_1 + new Vec2(160f, 28f);
      ...
      this._font.Draw("CANCEL", (float) ((double) p1_3.x + (double) vec2_2.x / 2.0 - (double) this._font.GetWidth("CANCEL") / 2.0), p1_3.y + 12f, Color.White, this.depth + 3);
      Graphics.DrawString(this._text, p1_1 + new Vec2(5f, 5f), Color.White, this.depth + 2);
    }
  }
}
