...
namespace DuckGame
{
  public static class Debug
  {
    ...
    public static void DrawLine(Vec2 p1, Vec2 p2, Color col, float width = 1f)
    {
      ...
      DuckGame.Graphics.Draw((Tex2D) Debug._blank, p1, new Rectangle?(), col, rotation, Vec2.Zero, new Vec2(length, width), SpriteEffects.None, new Depth(1f));
    }

    [Conditional("DEBUG")]
    public static void DrawRect(Vec2 p1, Vec2 p2, Color col) => DuckGame.Graphics.Draw((Tex2D) Debug._blank, p1, new Rectangle?(), col, 0.0f, Vec2.Zero, new Vec2((float) -((double) p1.x - (double) p2.x), (float) -((double) p1.y - (double) p2.y)), SpriteEffects.None, new Depth(1f));
  }
}
